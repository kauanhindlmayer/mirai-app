...
watch(
  ...
  async (newSelectedTeam) => {
    ...
    await teamStore.getBacklog(newSelectedTeam.id, selectedBacklogLevel.value)
  },
)
...
watch(
  ...
  async (newSelectedBacklogLevel) => {
    ...
    await teamStore.getBacklog(selectedTeam.value.id, newSelectedBacklogLevel)
  },
)

const nodes = computed(() => backlog.value.map(toNode))

watchEffect(() => console.log(nodes.value))
